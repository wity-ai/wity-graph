...
```js
{
  ...
  styles:       object,       // per-node visual overrides, UI layer interprets
...
| `update(x, y, xThreshold?, yThreshold?)` | Move placeholder; detects snap targets within threshold (default 300px) |
...
| `registerNodeType(name, config)` | Register a custom node type at runtime |
